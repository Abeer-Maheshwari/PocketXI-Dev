...
class SoundManager:
    # Audio manager for sound effects and background crowd ambiance
    ...
    def __init__(self, audio_path="assets/audio", master_volume=1.0):
        ...
        # Pygbag needs 24kHz audio in browser
        if self.is_web:
        ...
        if not pygame.mixer.get_init():
            try:
                ...
                print(f"Warning: Audio init failed ({error}).")

        if not pygame.mixer.get_init():
            return

        # Reserve channel 0 for crowd chants
    ...
    def load_assets(self):
        try:
            ...
            self.impacts.clear()
            self.chants.clear()
            print(f"Warning: Failed loading sound effects ({error}).")
